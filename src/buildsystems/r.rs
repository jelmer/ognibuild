use crate::buildsystem::{DependencyCategory,BuildSystem};
...
impl R {
    ...
    pub fn lint(&self, session: &dyn crate::session::Session, installer: &dyn crate::installer::Installer) -> Result<(), crate::buildsystem::Error> {
        let r_path = guaranteed_which(session, installer, "R").unwrap();
        session.command(vec![r_path.to_str().unwrap(), "CMD", "check"]).run_detecting_problems()?;
        Ok(())
    }
...
impl BuildSystem for R {
    ...
    fn dist(
        ...
        let r_path = guaranteed_which(session, installer, "R").unwrap();
        session.command(vec![r_path.to_str().unwrap(), "CMD", "build", "."]).run_detecting_problems()?;
    ...
    fn test(&self, session: &dyn crate::session::Session, installer: &dyn crate::installer::Installer) -> Result<(), crate::buildsystem::Error> {
        ...
        if session.exists(Path::new("run_tests.sh")) {
            session.command(vec!["./run_tests.sh"]).run_detecting_problems()?;
        } else if session.exists(Path::new("tests/testthat")) {
            session.command(vec![r_path.to_str().unwrap(), "-e", "testthat::test_dir('tests')"]).run_detecting_problems()?;
        }
        Ok(())
    }
    ...
    fn install(
        ...
        let r_path = guaranteed_which(session, installer, "R").unwrap();
        let mut args = vec![r_path.to_str().unwrap().to_string(), "CMD".to_string(), "INSTALL".to_string(), ".".to_string()];
    ...
    fn get_declared_dependencies(
        ...
        for s in description.suggests().unwrap_or_default() {
            ret.push((DependencyCategory::Build /* TODO */, Box::new(RPackageDependency::from_str(&s))));
        }
        for s in description.depends().unwrap_or_default() {
            ret.push((DependencyCategory::Build, Box::new(RPackageDependency::from_str(&s))));
        }
...
fn read_description<R: std::io::Read>(mut r: R) -> Result<r_description::RDescription, r_description::Error> {
